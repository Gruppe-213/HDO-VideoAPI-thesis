...
% Hver fanget exception registreres som et inkrement i Prometheus-telleren \texttt{hdo\_errors\_total}, etiketten settes til \texttt{exception.GetType().Name}. Det gir et grunnlag for å se hvilke typer feil som dominerer i produksjon uten å måtte parse logger. Logging skjer med to alvorlighetsgrader avhengig av statuskoden, slik at \texttt{4xx}-feil ikke fyller opp varsel-kanalen for ekte produksjonsfeil.

Hver fanget exception registreres som et inkrement i
Prometheus-telleren \texttt{hdo\_errors\_total}, der label-verdien
settes til \texttt{exception.GetType().Name}. Dette gir grunnlag for
å overvåke hvilke feiltyper som dominerer i produksjon uten å måtte
parse fritekstlogger. Loggalvorligheten differensieres etter
HTTP-statuskode: \texttt{5xx}-feil skrives med \texttt{LogError},
mens \texttt{4xx}-feil skrives med \texttt{LogWarning}, som vist i kodeliste~\ref{lst:logging}.

\begin{lstlisting}[language={[Sharp]C}, caption={Differensiert logging i \texttt{GlobalExceptionHandler}.}, label={lst:logging}]
// HDO-VideoAPI/Middleware/GlobalExceptionHandler.cs

public async ValueTask<bool> TryHandleAsync( ... )
{
    // ... (statuskode og feilmelding fra exception-type)
    ...
    // ... (RFC 7807 ProblemDetails-konstruksjon og JSON-respons))
}
\end{lstlisting}
...
Krav O1.1 \cite{hdo_kravspec} stiller krav om at API-nøkler, JWT-tokens og annen konfidensiell informasjon ikke skal forekomme i logger. Oppfyllelsen er ivaretatt ved at exception-klassene er utformet slik at \texttt{Message}-feltet kun inneholder informasjon som anses nødvendig for feilsøking, som ressurs-ID, tjenestenavn og responskropp fra eksterne tjenester. I \texttt{GlobalExceptionHandler} videresendes exception-objektet direkte til \texttt{LogError}/ \texttt{LogWarning}, slik at detaljert kontekst blir tilgjengelig i loggene. Dette innebærer at responsinnhold fra upstream-tjenester kan logges i sin helhet. Valget ble gjort for å forenkle diagnostisering og feilsøking, og ble vurdert og akseptert i samråd med HDO, under forutsetning av at responsene ikke inneholder API-nøkler, JWT-tokens eller annen sensitiv informasjon.
...
// Wrapper rundt alle API-kall: parser RFC 7807-feil til ApiError
...
// Formaterer feilmelding for visning
