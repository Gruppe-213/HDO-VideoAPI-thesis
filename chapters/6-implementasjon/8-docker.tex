...
Filtreringen skjer via \texttt{.dockerignore}, som ekskluderer \texttt{.git/}, \texttt{.env}, \texttt{bin/}, \texttt{obj/}, lokale \gls{ide}-filer og hemmeligheter. Det forhindrer at sensitive verdier eller utvikleroppsett havner i imaget.
...
For å hindre at en enkelt container uttømmer hostens ressurser, settes \texttt{deploy.resources.limits} per tjeneste: VideoAPI begrenses til 512~MB minne og én CPU-kjerne, Prometheus til 256~MB, og Grafana til 1~GB. Verdiene er forankret i den observerte ressursbruken under last- og integrasjonstesting, og gir tydelige grenser for systemovervåkningen.
