...
Med kjerneendringer menes endringer i \texttt{RoomsController},
\texttt{IVideoProvider} eller de offentlige modellene
(\texttt{VideoRoom}, \texttt{CreateRoomRequest},
\texttt{WebRtcLinkResponse}). Dette er komponentene som utgjør
API-kontrakten og controller-laget — de delene som ifølge
designmålet skal forbli uendret når en ny videomotor legges til.
Endringer i provider-klasser, konfigurasjon, DI-registrering og
tester teller ikke som kjerneendringer, siden disse per definisjon
er nødvendige ved enhver ny provider.
...
\bottomrule
\end{tabular}
\caption{Linjedekning per branch målt mot \texttt{develop}.
Kjerneendringer refererer til endringer i \texttt{RoomsController},
\texttt{IVideoProvider} og offentlige modeller. De 12 filene i
\texttt{feature/daily-provider} er utelukkende nye filer i en egen
\texttt{Daily/}-undermappe — ingen eksisterende filer ble endret.}
